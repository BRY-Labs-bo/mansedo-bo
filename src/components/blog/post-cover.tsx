...
// - Si el post tiene coverImage (URL), la renderiza con lazy loading nativo.
// - Si no, muestra un motivo de marca: fondo navy-deep + patrón geométrico
//   dorado + la pica del isologo grande al centro, con la categoría arriba.
// Se usa en tarjetas del listado, tarjetas del home y en el artículo.
...
export function PostCover({
  ...
  if (src) {
    // Imagen real: <img> plano con lazy/eager. Evita configurar remotePatterns
    // en next.config y sirve URLs externas sin fricción.
    return (
...
function PostCoverPattern({
  ...
  return (
    ...
    >
      ...
      {/* Categoría opcional (sólo se pinta si viene por prop; por defecto la
          info ya aparece en el body de la tarjeta y no se duplica acá). */}
      {category && (
